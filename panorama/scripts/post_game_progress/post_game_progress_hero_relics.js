...
class AnimateHeroRelicProgressAction extends RunSequentialActions {
    ...
    start() {
        ...
        this.panel.SetDialogVariable(
            "current_progress",
            GameUI.ConstructNumberString({ Number: this.data.starting_value, PreferIntegral: true, Precision: 1 }),
        );
        this.panel.SetDialogVariable(
            "increment",
            GameUI.ConstructNumberString({
                Number: this.data.ending_value - this.data.starting_value,
                PreferIntegral: true,
                Precision: 1,
            }),
        );
        ...
        switch (this.data.primary_attribute) {
            case DOTA_ATTRIBUTE_STRENGTH:
                this.panel.AddClass("PrimaryAttributeStrength");
                break;
            case DOTA_ATTRIBUTE_AGILITY:
                this.panel.AddClass("PrimaryAttributeAgility");
                break;
            case DOTA_ATTRIBUTE_INTELLECT:
                this.panel.AddClass("PrimaryAttributeIntellect");
                break;
        }
        ...
        while (nCurrentValue < this.data.ending_value) {
            ...
            if (nValueToNextLevel <= nRemainingProgress) {
                ...
                this.actions.push(
                    new SetDialogVariableStringAction(this.panel, "relic_level", (nCurrentLevel + 1).toString()),
                );
                ...
                this.actions.push(
                    new AnimateProgressBarAction(
                        progressBar,
                        nCurrentLevelValue,
                        this.data.ending_value % nValuePerLevel,
                        0.3,
                    ),
                );
...
class AnimateHeroRelicsScreenAction extends RunSequentialActions {
    ...
    start() {
        ...
        this.actions.push(new WaitAction(0.5));
        var stagger = new RunStaggeredActions(0.1);
