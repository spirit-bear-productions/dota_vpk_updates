    ...
    start() {
        ...
        actions.push(
            new AddScreenLinkAction(
                panel,
                "MonsterHunterProgress",
                "#DOTA_PlusPostGame_MonsterHunterProgress",
                function () {
        ...
        if (bHunterDuel) {
            ...
            objectiveIcon.SetImage(
                "file://{images}/events/monster_hunter/artwork/quest_side_objective_icon_withbg_gloves.psd",
            );
        }
        if (this.data.hunt_rewards.length > 0) {
            ...
            for (let i = 0; i < this.data.hunt_rewards.length; i++) {
                ...
                materialPanel.SetMaterialEarnedReason(
                    bHunterDuel ? 3 : 1,
                    this.data.target_hero_id,
                    this.data.hunt_rewards[i].rewarded,
                );
            ...
            for (let i = 0; i < this.data.denial_rewards.length; i++) {
                if (this.data.denial_rewards[i].hero_id == this.data.target_hero_id) {
                    let materialContainerPanel = $.CreatePanel(
                        "Panel",
                        materialRewardsListPanels[0],
                        "Material" + i,
                        {},
                    );
                    materialContainerPanel.BLoadLayoutSnippet("MonsterHunterMaterial");
