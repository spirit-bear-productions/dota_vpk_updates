...
function GetNextLevel(nCurrentLevel, event_levels) {
    if (event_levels.length < 2) return nCurrentLevel;
...
function GetStartingLevelForPoints(nPoints, bpPointsPerLevel, event_levels) {
    ...
    for (
        var nLevel = 0;
        nPoints > nLevel * bpPointsPerLevel && nLevel < 100;
        nLevel = GetNextLevel(nLevel, event_levels)
    ) {
        nReturnLevel = nLevel;
    }
    ...
    constructor(panel, eventId, bpPointsStart, bpPointsPerLevel, event_levels, bpPointsAdd) {
        ...
        while (bpRemaining > 0) {
            ...
            if (bpToAnimate > 0) {
                this.actions.push(
                    new SkippableAction(
                        new AnimateBattlePointsIncreaseAction(
                            panel,
                            bpToAnimate,
                            bpEarnedOnRow,
                            bpEarned,
                            bpLevel,
                            "Frostivus.ui_point_increment",
                        ),
                    ),
                );
...
class AnimateFrostivus2023SubpanelAction extends RunSequentialActions {
    ...
    start() {
        ...
        this.actions.push(
            new AnimateFrostivus2023LevelsAction(
                ownerPanel,
    ...
    start() {
        ...
        this.actions.push(
            new AddScreenLinkAction(
                panel,
                "Frostivus2023Progress",
                "#DOTA_PlusPostGame_Frostivus2023Progress",
                function () {
