...
class WaitForRatingStateChange extends WaitForConditionAction {
    constructor(panel) {
        super(() => {
            ...
            if (goodRatingButton.BHasClass("Selected")) return true;

            var badRatingButton = panel.FindChildInLayoutFile("BadRatingButton");
            if (badRatingButton.BHasClass("Selected")) return true;

            var abusiveRatingButton = panel.FindChildInLayoutFile("AbusiveRatingButton");
            if (abusiveRatingButton.BHasClass("Selected")) return true;
...
class WaitForSurveyStateChange extends WaitForConditionAction {
    constructor(panel) {
        super(() => {
            ...
            if (!goodRatingContainer.enabled) return true;

            var badRatingContainer = panel.FindChildInLayoutFile("BadRatingContainer");
            if (!badRatingContainer.enabled) return true;

            var skipButton = panel.FindChildInLayoutFile("SkipButton");
            if (skipButton.BHasClass("Selected")) return true;
...
class AnimateCoachRatingScreenAction extends RunSequentialActions {
    ...
    start() {
        ...
        var SubmitRating = function (strRating, strReason) {
            if (action_data.match_id == "0") return;

            $.DispatchEvent(
                "DOTASubmitCoachRating",
                action_data.match_id,
                rating_data.coach_account_id,
                strRating,
                strReason,
            );
        ...
        $.RegisterEventHandler(
            "PostGameProgressConfirmAbusiveCoachRatingFinished",
            panel,
            function (bSubmit, strReason) {
                if (bSubmit) {
        ...
        countdownActions.actions.push(
            new AnimateDialogVariableIntAction(panel, "countdown_seconds", flCountdownDuration, 0, flCountdownDuration),
        );
        countdownActions.actions.push(
            new AnimateProgressBarAction(countdownProgressBar, flCountdownDuration, 0, flCountdownDuration),
        );
