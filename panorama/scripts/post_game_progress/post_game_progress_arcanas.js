...
class AnimateDrowRangerArcanaScreenAction extends RunSequentialActions {
    ...
    start() {
        ...
        if (sHeader == sHeaderLoc) {
            sHeader = $.Localize("#DOTA_ArcanaProgress_Header_generic", panel);
        }
        if (sTitle == sTitleLoc) {
            sTitle = $.Localize("#DOTA_ArcanaProgress_Title_generic", panel);
        }
        if (sDescription == sDescriptionLoc) {
            sDescription = $.Localize("#DOTA_ArcanaProgress_Description_generic", panel);
        }
        if (sProgressBar == sProgressBarLoc) {
            sProgressBar = $.Localize("#DOTA_ArcanaProgress_ProgressBar_generic", panel);
        }
        if (sIncrement == sIncrementLoc) {
            sIncrement = $.Localize("#DOTA_ArcanaProgress_Increment_generic", panel);
        }
        ...
        if (sndStinger) {
            this.actions.push(
                new RunFunctionAction(function () {
                    $.DispatchEvent("PlaySoundEffect", sndStinger);
                }),
            );
        }
...
class AnimateRubickArcanaScreenAction extends RunSequentialActions {
    ...
    start() {
        ...
        this.actions.push(new SkippableAction(new WaitAction(0.5)));
        this.actions.push(
            new AddScreenLinkAction(panel, "RubickArcanaProgress", "#DOTA_PlusPostGame_RubickArcanaProgress"),
        );
        this.actions.push(new ActionWithTimeout(new WaitForClassAction(heroModel, "SceneLoaded"), 3.0));
        this.actions.push(new SkippableAction(new WaitAction(0.5)));
        this.actions.push(new AddClassAction(panel, "ShowProgress"));
...
class AnimateWraithKingArcanaScreenAction extends RunSequentialActions {
    ...
    start() {
        ...
        this.actions.push(
            new AddScreenLinkAction(panel, "WraithKingArcanaProgress", "#DOTA_PostGame_WraithKingArcanaProgress"),
        );
        this.actions.push(new SkippableAction(new WaitAction(1.0)));
        this.actions.push(
            new RunFunctionAction(function () {
                $.DispatchEvent("PlaySoundEffect", "wraith_king_arcana_postgame_vo");
            }),
        );
