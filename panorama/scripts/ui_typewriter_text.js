...
function SetFinished() {
    ...
    if (strText.length == 1 && strText.charAt(0) === " ") strText = "";
...
function GetNextCharInfo(strInput, nSplitIndex, flOffset) {
    ...
    if (chCur === "\\") {
        ...
        if (chNext === "?" || chNext === "!" || chNext === ".") {
            ...
            if (chNext === "-" || chNext === "+") flOffset += flMult;
            else g_typeWriterEffect.m_flCharDelayMult = flMult;
    ...
    if (!bEscaped) {
        if (chCur === "." || chCur === "。" || chCur === "…") {
            ...
            bPlaySounds = false;
            flDelayMult *= 15;
        } else if (chCur === "!" || chCur === "?" || chCur === "！" || chCur === "？") {
            // sChinese
            bAnimateCursor = false;
            bPlaySounds = false;
            flDelayMult *= 20;
...
function UpdateEffects() {
    ...
    for (var i = 0; i < nCharIncrement; ++i) {
        g_typeWriterEffect.m_nCharacterIndex = AdvanceAndEatHTML(
            g_typeWriterEffect.m_strText,
...
function OnTextChanged(
    ...
    if (bInstant) {
        ...
        if (flVODuration > 0) {
            ...
            if (g_typeWriterEffect.m_flCharacterDelay < 0.01) g_typeWriterEffect.m_flCharacterDelay = 0.01;
