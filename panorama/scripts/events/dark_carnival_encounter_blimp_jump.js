        ...
        generations: [
            {
                generation: {
                    amount: [5, 8],
                    platformTypes: [[PlatformType_t.Normal, 1]],
                    GapY: [30, 45],
                },
                weight: 1,
            },
    ...
    },
    {
        ...
        generations: [
            ...
            },
            {
                generation: {
                    amount: [3, 5],
                    platformTypes: [[PlatformType_t.Moving, 1]],
                    attachmentChance: 0.1,
                    attachmentChances: [
                        [PlatformAttachmentType_t.Spring, 1],
                        [PlatformAttachmentType_t.Trampoline, 0.5],
        ...
        generations: [
            ...
            },
            {
                generation: {
                    ...
                    attachmentChances: [[PlatformAttachmentType_t.Spikes, 1]],
                    enemyChance: 0.3,
                    GapY: [60, 80],
                },
                weight: 0.4,
            ...
            },
            {
                generation: {
                    amount: [3, 6],
                    platformTypes: [[PlatformType_t.Moving, 0.3]],
                    attachmentChance: 0.5,
                    attachmentChances: [
    ...
    // Infinite
    {
        ...
        generations: [
            ...
            },
            {
                generation: {
                    amount: [6, 10],
                    platformTypes: [[PlatformType_t.WallJump, 0.3]],
                    enemyChance: 0.3,
                    GapY: [70, 90],
                },
                weight: 0.4,
            },
            {
                generation: {
                    amount: [2, 3],
                    platformTypes: [[PlatformType_t.Balloon, 1]],
                    enemyChance: 0.3,
                    GapY: [60, 90],
                },
                weight: 0.2,
...
class Player_t extends GameObject_t {
    ...
    IsRocketing() {
        return (
            this.rocketStartedTimestamp != null &&
    ...
    update(dt) {
        if (this.startWinningTimestamp != null) {
            ...
            if (this.diedByHazard) {
                ...
                if (delta < 0.3) this.element.SwitchClass("Stance", "Oof2");
                else if (delta < 0.4) this.element.SwitchClass("Stance", "Oof");
                else if (delta < 0.5) this.element.SwitchClass("Stance", "Oof2");
                else if (!IsObjectTooLow(this)) {
                    if (!this.startedDeathLaunch) {
                        this.acceleration = { x: 0, y: PLAYER_GRAVITY * 1.5 };
                        this.velocity = {
                            x: RandomInt(150, 200) * (this.position.x / viewportWidth < 0.5 ? 1 : -1),
                            y: 400,
                        };
        ...
        if (upPressed && !this.projectileKeyDown && this.projectileCooldownRemaining <= 0) {
            new Projectile_t(
                this.position,
                { x: this.velocity.x, y: this.IsRocketing() ? PROJECTILE_SPEED + ROCKET_SPEED / 2 : PROJECTILE_SPEED },
                { x: 0, y: PROJECTILE_GRAVITY },
            );
...
class Platform_t extends GameObject_t {
    ...
    checkCollisionWithPlayer(player) {
        ...
        if (HasFlag(this.type, PlatformType_t.Balloon) && IsOverlapping(player, this)) {
...
function CreateBlimp(position) {
    ...
    platform.position = {
        x: position.x + 20,
        y: position.y - BLIMP_HEIGHT / 2 - DEFAULT_FINISH_PLATFORM_HEIGHT / 2 + 2,
    };
...
function Initialize(bInfinite) {
    ...
    if (DEBUG_LEVEL) {
        ...
        }
        {
            const enemy = CreateEnemy(
                { x: 450, y: 500 },
                ENEMY_PLATFORM_WIDTH,
                ENEMY_PLATFORM_HEIGHT,
                EnemyType_t.PlatformEnemy,
                null,
            );
            enemy.update(0);
            enemy.render();
        ...
        }
        {
            const platform = CreatePlatform(
                { x: 100, y: 100 },
                TUTORIAL_PLATFORM_WIDTH,
                TUTORIAL_PLATFORM_HEIGHT,
                PlatformType_t.Tutorial,
            );
        ...
        CreatePlatform(
            { x: SPAWN_POSITION_X, y: FIRST_PLATFORM_HEIGHT },
            DEFAULT_PLATFORM_WIDTH,
            DEFAULT_PLATFORM_HEIGHT,
            PlatformType_t.Normal,
        );
...
function UpdatePanelClass() {
    ...
    $.GetContextPanel().SwitchClass(
        "ConfettiState",
        gameState?.player?.startWinningTimestamp != null ? "ShowConfetti" : "",
    );
    $.GetContextPanel().SwitchClass(
        "InfiniteOrStory",
        gameState?.infiniteMode ? "BlimpJump_Infinite" : "BlimpJump_Story",
    );
}
function PickMusic() {
    if (blimpJumpState == BlimpJumpState_t.Won) return "dark_carnival.blimp_jump.music.victory";
    if (blimpJumpState == BlimpJumpState_t.Dead) return "dark_carnival.blimp_jump.sound.null";
    if (blimpJumpState == BlimpJumpState_t.MainMenu) return "dark_carnival.blimp_jump.music.mainmenu";
    ...
    switch (blimpJumpState) {
        case BlimpJumpState_t.MainMenu: {
            ...
            if (slarkPanel) {
                ...
                slarkPanel.SwitchClass(
                    "Stance",
                    isBounceGoingDown(Game.GetGameTime(), 700, 1200) ? "Falling" : "Jumping",
                );
            }
            return;
        }
...
function IsOverlapping(a, b) {
    ...
    return (
        Math.abs(a.position.x - b.position.x) < (aWidth + bWidth) * 0.5 &&
...
function IsLandingCollision(player, gameObject) {
    ...
    const horizontal = Math.abs(player.position.x - gameObject.position.x) < (playerWidth + gameObjectWidth) / 2;
    if (!horizontal) {
        return false;
    }
    ...
    const swept = playerPrevBottom >= platformPrevTop && playerCurrBottom <= platformTop;
    return swept;
}
function IsHorizontalCollision(player, gameObject) {
    ...
    const vertical = Math.abs(player.position.y - gameObject.position.y) < (player.height + gameObject.height) * 0.5;
...
function PickWeightedAttachmentType(attachmentChances) {
    const filtered = attachmentChances.filter(
        ([type]) => type != PlatformAttachmentType_t.Rocket || gameState.infiniteMode,
    );
    let totalWeight = 0;
    for (const [, weight] of filtered) {
        totalWeight += weight;
...
function SpawnGeneration(generation) {
    ...
    for (let i = 0; i < amount; i++) {
        if (!gameState.infiniteMode && !gameState.reachedBlimp && currentY >= STORY_MODE_GOAL) {
            ...
            gameState.blimp = CreateBlimp({ x: viewportWidth / 2, y: currentY + BLIMP_HEIGHT * 0.75 });
            return;
        }
        ...
        let getXCandidate = () => {
            ...
            if (gameState.verticalAvoidRange[0] < currentY && currentY < gameState.verticalAvoidRange[1]) {
                x = randomExcludingRange(
                    width / 2,
                    viewportWidth - width / 2,
                    gameState.horizontalAvoidRange[0],
                    gameState.horizontalAvoidRange[1],
                );
            ...
            if (
                ...
                enemyType =
                    Math.random() < 0.5
                        ...
                        : Math.random() < 0.5
                        ? EnemyType_t.FlyEnemy
                        : EnemyType_t.FloatEnemy;
            }
            switch (enemyType) {
                case EnemyType_t.FlyEnemy:
                    CreateEnemy(
                        {
                            x: RandomInt(ENEMY_FLYING_WIDTH, viewportWidth - ENEMY_FLYING_WIDTH),
                            y: currentY + ENEMY_FLY_SPAWN_PADDING,
                        },
                        ENEMY_FLYING_WIDTH,
                        ENEMY_FLYING_HEIGHT,
                        EnemyType_t.FlyEnemy,
                ...
                case EnemyType_t.FloatEnemy:
                    ...
                    CreatePlatform(
                        { x: x, y: y },
                        DEFAULT_BALLOON_WIDTH,
                        DEFAULT_BALLOON_HEIGHT,
                        PlatformType_t.Balloon,
                    );
                    gameState.horizontalAvoidRange = [x - DEFAULT_BALLOON_WIDTH * 1.5, x + DEFAULT_BALLOON_WIDTH * 1.5];
                    gameState.verticalAvoidRange = [
                        y - DEFAULT_BALLOON_HEIGHT / 2 - ENEMY_FLOAT_SPAWN_PADDING,
                        y + DEFAULT_BALLOON_HEIGHT / 2 + ENEMY_FLOAT_SPAWN_PADDING,
                    ];
                ...
                case EnemyType_t.PlatformEnemy:
                    CreateEnemy(
                        { x: RandomInt(ENEMY_PLATFORM_WIDTH, viewportWidth - ENEMY_PLATFORM_WIDTH), y: currentY },
                        ENEMY_PLATFORM_WIDTH,
                        ENEMY_PLATFORM_HEIGHT,
                        EnemyType_t.PlatformEnemy,
                        platform,
        ...
        if (tutorialSpawn == TutorialType_t.AttackTutorial) {
            ...
            gameState.verticalAvoidRange = [
                currentY,
                balloonSpawn + DEFAULT_BALLOON_HEIGHT + ENEMY_FLOAT_SPAWN_PADDING,
            ];
...
function HandleJumpTutorialSpawn(x, currentY) {
    ...
    x = 400;
    const tutorialPlatform = CreatePlatform(
        { x: x, y: currentY },
        TUTORIAL_PLATFORM_WIDTH,
        TUTORIAL_PLATFORM_HEIGHT,
        PlatformType_t.Tutorial,
...
function GetCurrentGeneration() {
    ...
    for (const entry of levelGeneration.generations) {
        roll -= entry.weight;
        if (roll <= 0) return entry.generation;
    }
    ...
    if ((type & PlatformType_t.Normal) != 0) {
        if (Math.random() < 0.07) {
            const additionalType =
                Math.random() < 0.7 ? PlatformAttachmentType_t.Spring : PlatformAttachmentType_t.Trampoline;
...
function CleanupDestroyedObjects() {
    gameState.gameObjects
        .filter((gameObject) => gameObject.destroyed)
        .forEach((gameObject) => {
            gameObject.element?.DeleteAsync(0.0);
        });
...
function EndGame(bWon) {
    ...
    $.GetContextPanel().SetDialogVariableLocString(
        "post_game_title",
        bWon ? "#DOTA_DarkCarnival_BlimpJump_PostGame_Title_Won" : "#DOTA_DarkCarnival_BlimpJump_PostGame_Title_Lost",
    );
    $.GetContextPanel().SetDialogVariableLocString(
        "post_game_subtitle",
        bWon
            ? "#DOTA_DarkCarnival_BlimpJump_PostGame_Subtitle_Won"
            : gameState.infiniteMode
            ? "#DOTA_DarkCarnival_BlimpJump_PostGame_Subtitle_Lost"
            : "#DOTA_DarkCarnival_BlimpJump_PostGame_Subtitle_LostStoryMode",
    );
