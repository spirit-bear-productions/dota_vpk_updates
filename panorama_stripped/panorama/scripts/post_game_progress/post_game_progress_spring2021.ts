...
class AnimateSpring2021LevelsAction extends RunSequentialActions {
    ...
    constructor(panel: Panel_t, eventId: number, bpPointsStart: number, bpPointsPerLevel: number, bpPointsAdd: number) {
        ...
        (panel.FindChildInLayoutFile("BattlePassLevelShield") as DOTAEventLevelShield_t).SetEventLevel(
            this.eventId,
            battleLevelStart,
        );
...
class AnimateSpring2021WeeklyProgressIncreaseAction extends RunParallelActions {
    ...
    start(): void {
        ...
        this.actions.push(
            new AnimateProgressBarWithMiddleAction(levelProgressBar, minProgressValue, maxProgressValue, duration),
        );
...
class AnimateSpring2021WeeklyProgressLevels extends RunSequentialActions {
    constructor(
        ...
        nStartValue: number,
        nProgress: number,
        levelThresholds: BPSpring2021WeeklyQuest_LevelThreshold_t[],
    ) {
        super();
        ...
        while (nProgressRemaining > 0) {
...

            {
                if (nNextThresholdIndex < levelThresholds.length) {
                    var nNextProgressThreshold =
                        levelThresholds[Math.min(nNextThresholdIndex, levelThresholds.length - 1)].threshold;
                    this.actions.push(
                        new SkippableAction(
                            new AnimateSpring2021WeeklyProgressIncreaseAction(
                                panel,
...
var AddNewWeeklyProgressPanelSpring2021 = function (
    ...
    seq.actions.push(
        new AnimateSpring2021WeeklyProgressLevels(
            panel as WeeklyProgressPanel_t,
...
class AnimateSpring2021ScreenAction extends RunSequentialActions {
    ...
    start() {
        ...
        this.actions.push(
            new AddScreenLinkAction(panel, "Spring2021Progress", "#DOTA_PlusPostGame_Spring2021Progress", function () {
        ...
        if (this.data.spring_2021_progress.actions_granted != null) {
            ...
            if (++panelCount > kMaxPanels) progressPanel.RemoveClass("Visible");
        }
        ...
        if (this.data.spring_2021_progress.cavern_crawl != null) {
            ...
            const subpanelAction = new AnimateCavernCrawlSubpanelAction(
                cavernPanel,
                panel,
                this.data.spring_2021_progress.cavern_crawl,
                startingPointsToAdd,
            );
            startingPointsToAdd += subpanelAction.total_points;
            subPanelActions.actions.push(subpanelAction);
        ...
        this.actions.push(
            new AnimateSpring2021LevelsAction(
                panel,
                this.data.spring_2021_progress.battle_points_event_id,
