...
function AddScreenLink(
    ...
    link.SetPanelEvent("onmouseover", function () {
        $.DispatchEvent("UIShowTextTooltip", link, tooltipText);
    });
    link.SetPanelEvent("onmouseout", function () {
        $.DispatchEvent("UIHideTextTooltip", link);
    });
...
class AddScreenLinkAction extends RunFunctionAction {
    constructor(screenPanel: Panel_t, linkClass: string, tooltipText: string, activateFunction?: () => void) {
        super(function () {
            AddScreenLink(screenPanel, linkClass, tooltipText, activateFunction);
        });
    }
}
...
function SetSkippingAhead(bSkipAhead: boolean) {
    if (g_bSkipAheadActions == bSkipAhead) return;
...
function StopSkippingAhead(): void {
    SetSkippingAhead(false);
}
function StartSkippingAhead(): void {
...
class OptionalSkippableAction extends BaseAction {
    ...
    update(): boolean {
        if (this.innerActionStarted) return this.innerAction.update() && !IsSkippingAhead();
