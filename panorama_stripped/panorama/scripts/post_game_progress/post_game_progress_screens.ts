...
interface DOTACavernCrawlPostGameProgress_t extends Panel_t {
    ...
    ShowPostGameProgress(
        nEventID: number,
        nAccountID: number,
        nVariant: number,
        nHeroIDCompleted: number,
        bTurboMode: boolean,
    ): void;
...
function UpdateSubpanelTotalPoints(
    panel: Panel_t,
    ownerPanel: Panel_t,
    bpAmount: number,
    bpStartingSubTotal: number,
    displayOnly: boolean,
) {
...
class AnimateCavernCrawlSubpanelAction extends RunSequentialActions {
    ...
    constructor(
        panel: Panel_t,
        ownerPanel: Panel_t,
        cavern_data: BattlePassCavernCrawlProgress_t,
        startingPoints: number,
    ) {
        super();
    ...
    start(): void {
        ...
        if (this.show_premium_weekly_progress) {
            ...
            (function (me) {
                me.actions.push(
                    new RunFunctionAction(function () {
                        var progressBar = me.panel.FindChildInLayoutFile(
                            "EventGameWeeklyPremiumProgress",
                        ) as ProgressBarWithMiddle_t;
...
class AnimateEventGameNemesticeSubpanelAction extends RunSequentialActions {
    ...
    constructor(
        panel: Panel_t,
        ownerPanel: Panel_t,
        event_game_nemestice: BattlePassEventGameNemestice_t,
        startingPoints: number,
    ) {
        super();
...
class AnimateWeeklyChallengeSubpanelAction extends RunSequentialActions {
    ...
    constructor(
        panel: Panel_t,
        ownerPanel: Panel_t,
        weekly_challenge: BattlePassWeeklyChallenge_t,
        startingPoints: number,
    ) {
        super();
...
class AnimateGuildSubpanelAction extends RunSequentialActions {
    ...
    constructor(
        ...
        if (guild_progress.guild_challenge != null) {
            ...
            var challengeProgressBar = panel.FindChildInLayoutFile(
                "GuildChallengeProgressBar",
            ) as ProgressBarWithMiddle_t;
...
class AnimateFrostivusScreenAction extends RunSequentialActions {
    ...
    start(): void {
        ...
        panel.SetDialogVariableInt("current_level_bp", bpLevelStart);
        panel.SetDialogVariableInt("bp_to_next_level", bpLevelNext);
        (panel.FindChildInLayoutFile("Frostivus2018LevelShield") as DOTAEventLevelShield_t).SetEventLevel(
            this.data.frostivus_progress.battle_points_event_id,
            battleLevelStart,
        );
        ...
        while (bpRemaining > 0) {
            ...
            (function (me, battleLevelInternal) {
                me.actions.push(
                    new RunFunctionAction(function () {
                        var levelShield = panel.FindChildInLayoutFile(
                            "Frostivus2018LevelShield",
                        ) as DOTAEventLevelShield_t;
...
class AnimateEventPointsScreenAction extends RunSequentialActions {
    ...
    start(): void {
        ...
        panel.SetDialogVariableInt("current_level_bp", bpLevelStart);
        panel.SetDialogVariableInt("bp_to_next_level", bpLevelNext);
        (panel.FindChildInLayoutFile("EventPointsLevelShield") as DOTAEventLevelShield_t).SetEventLevel(
            this.data.event_points_progress.battle_points_event_id,
            battleLevelStart,
        );
