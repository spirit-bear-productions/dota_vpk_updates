...
class AnimateHeroBadgeLevelRewardAction extends RunSequentialActions {
    ...
    start() {
        if (this.data.reward_type == HERO_BADGE_LEVEL_REWARD_TIER) {
            ...
            reward.SetDialogVariable(
                "all_chat_prefix",
                this.data.all_chat ? $.Localize("#dota_all_chat_label_prefix") : "",
            );
...
class AnimateHeroBadgeLevelScreenAction extends RunSequentialActions {
    ...
    start() {
        ...
        if (this.data.hero_badge_progress_old) {
            this.actions.push(
                new AddScreenLinkAction(panel, "HeroBadgeProgress", "#DOTA_PlusPostGame_HeroProgress", function () {
            ...
            for (var i = 0; i < this.data.hero_badge_progress_old.length; ++i) {
                ...
                while (xpRemaining > 0) {
                    ...
                    if (xpToAnimate > 0) {
                        this.actions.push(
                            new SkippableAction(
                                new AnimateHeroBadgeXPIncreaseActionOld(
                                    panel,
                                    this.data.hero_badge_progress_old[i],
                                    xpToAnimate,
                                    xpEarnedOnRow,
                                    xpEarned,
                                    xpLevel,
            ...
            if (this.data.dota_plus_progress_old !== undefined) {
                if (
                    this.data.dota_plus_progress_old.tips !== undefined &&
                    this.data.dota_plus_progress_old.tips.length != 0
                ) {
                    var nShardTips = 0;
                    ...
                    this.actions.push(
                        new AnimateShardRewardActionOld(panel, "#DOTA_PlusPostGame_PlayerTips", nShardTips),
                    );
                }

                if (this.data.dota_plus_progress_old.victory_prediction_shard_reward > 0) {
                    this.actions.push(
                        new AnimateShardRewardActionOld(
                            panel,
                            "#DOTA_PlusPostGame_VictoryPrediction",
                            this.data.dota_plus_progress_old.victory_prediction_shard_reward,
                        ),
                    );
                }
                ...
                if (this.data.dota_plus_progress_old.cavern_crawl !== undefined) {
                    this.actions.push(
                        new AnimateShardRewardActionOld(
                            ...
                            this.data.dota_plus_progress_old.cavern_crawl.shard_amount,
                        ),
                    );
                }
                ...
                if (this.data.dota_plus_progress_old.role_call_shard_reward > 0) {
                    this.actions.push(
                        new AnimateShardRewardActionOld(
                            ...
                            this.data.dota_plus_progress_old.role_call_shard_reward,
                        ),
                    );
                }
            }
            ...
            if (this.data.hero_relics_progress_old.length > 0) {
                ...
                this.actions.push(
                    new AddScreenLinkAction(
                        panel,
                        "HeroRelicsProgress",
                        "#DOTA_PlusPostGame_RelicsProgress",
                        function () {
                ...
                for (var i = 0; i < this.data.hero_relics_progress_old.length; ++i) {
                    stagger.actions.push(
                        new AnimateSingleRelicProgressAction(this.data.hero_relics_progress_old[i], relicsList),
                    );
                }
