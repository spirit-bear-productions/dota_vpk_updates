...
class AnimateBattlePass2022WeeklyQuestEntryAction extends RunSequentialActions {
    ...
    constructor(
        ...
        const progressBar = this.entryPanel.FindChildInLayoutFile(
            "BattlePass2022WeeklyProgressBar",
    ...
    start(): void {
        ...
        for (let i = 0; i < this.actionGranted.level_thresholds.length; ++i) {
            ...
            par.actions.push(
                new AnimateProgressBarWithMiddleAction(progressBar, startValue, nextAnimateValue, duration),
            );
...
class AnimateBattlePass2022WeeklyQuestsAction extends RunSequentialActions {
    ...
    constructor(
        ...
        const pipBar = this.sectionPanel.FindChildInLayoutFile(
            "BattlePass2022WeeklyQuestsPipBar",
        ) as DOTAWeeklyQuestPipBar_International2022_t;
    ...
    start(): void {
        ...
        if (this.actionsGranted != null) {
            for (let i = 0; i < this.actionsGranted.length; ++i) {
                this.actions.push(
                    new AnimateBattlePass2022WeeklyQuestEntryAction(questList, pipBar, this.actionsGranted[i]),
                );
            }
        }
...
class AnimateBattlePass2022CavernCrawlAction extends RunSequentialActions {
    ...
    start(): void {
        (this.sectionPanel.FindChildInLayoutFile("BattlePass2022CavernCrawlHeroMovie") as DOTAHeroMovie_t).heroid =
            this.cavernCrawlData.hero_id;
        ...
        par.actions.push(
            new AnimateDialogVariableIntAction(
                this.sectionPanel,
                "cavern_crawl_points_earned",
                0,
                this.cavernCrawlData.bp_amount,
                duration,
            ),
        );
        par.actions.push(new PlaySoundForDurationAction("XP.Count", duration));
...
class AnimateBattlePass2022WeeklyBonusAction extends RunSequentialActions {
    ...
    start(): void {
        ...
        let progressBar = this.sectionPanel.FindChildInLayoutFile(
            "BattlePass2022WeeklyBonusProgressBar",
        ) as ProgressBarWithMiddle_t;
        progressBar.min = 0;
        ...
        par.actions.push(
            new AnimateDialogVariableIntAction(
                this.sectionPanel,
                "weekly_bonus_points_earned",
                0,
                this.weeklyBonusData.points_earned,
                duration,
            ),
        );
        par.actions.push(
            new AnimateDialogVariableIntAction(
                this.sectionPanel,
                "weekly_bonus_points_current",
                this.weeklyBonusData.start_points,
                this.weeklyBonusData.start_points + this.weeklyBonusData.points_earned,
                duration,
            ),
        );
        par.actions.push(
            new AnimateProgressBarWithMiddleAction(
                progressBar,
                this.weeklyBonusData.start_points,
                this.weeklyBonusData.start_points + this.weeklyBonusData.points_earned,
                duration,
...
class AnimateBattlePass2022CandyPointsAction extends RunSequentialActions {
    ...
    start(): void {
        ...
        let progressBar = this.sectionPanel.FindChildInLayoutFile(
            "BattlePass2022CandyPointsProgressBar",
        ) as ProgressBarWithMiddle_t;
        progressBar.min = 0;
        ...
        while (pointsRemaining > 0) {
            ...
            let targetBags =
                targetValue == this.candyPointData.points_per_bag ? currentBags + 1 : (currentBags as number);
...
class AnimateBattlePass2022ScreenAction extends RunSequentialActions {
    ...
    start() {
        ...
        this.actions.push(
            new AddScreenLinkAction(panel, "BattlePass2022Progress", "#DOTA_BattlePassPostGame_Progress"),
        );

        if (this.data.battle_pass_2022_progress.owned) {
            this.actions.push(
                new SkippableAction(
                    new AnimateBattlePass2022WeeklyQuestsAction(
        ...
        if (this.data.battle_pass_2022_progress.cavern_crawl != null) {
            ...
            this.actions.push(
                new SkippableAction(
                    new AnimateBattlePass2022CavernCrawlAction(
                        section,
                        this.data.battle_pass_2022_progress.cavern_crawl,
                    ),
                ),
            );
            this.actions.push(new SkippableAction(new WaitAction(0.5)));
        ...
        if (
            this.data.battle_pass_2022_progress.weekly_bonus != null &&
            this.data.battle_pass_2022_progress.weekly_bonus.points_earned > 0
        ) {
            const section = panel.FindChildInLayoutFile("BattlePass2022WeeklyBonusProgress");
            section.AddClass("SectionHasData");
            this.actions.push(
                new SkippableAction(
                    new AnimateBattlePass2022WeeklyBonusAction(
                        section,
                        this.data.battle_pass_2022_progress.weekly_bonus,
                    ),
                ),
            );
            this.actions.push(new SkippableAction(new WaitAction(0.5)));
        ...
        if (this.data.battle_pass_2022_progress.candy_points != null) {
            ...
            this.actions.push(
                new SkippableAction(
                    new AnimateBattlePass2022CandyPointsAction(
                        section,
                        this.data.battle_pass_2022_progress.candy_points,
                    ),
                ),
            );
            this.actions.push(new SkippableAction(new WaitAction(0.5)));
