...
class LerpDepthOfFieldAction extends LerpAction {
    ...
    applyProgress(progress: number) {
        this.panel.FireEntityInput(
            this.cameraName,
            "SetDOFNearBlurry",
            Lerp(progress, this.nearBlurryDistanceStart, this.nearBlurryDistanceEnd),
        );
        this.panel.FireEntityInput(
            this.cameraName,
            "SetDOFNearCrisp",
            Lerp(progress, this.nearCrispDistanceStart, this.nearCrispDistanceEnd),
        );
        this.panel.FireEntityInput(
            this.cameraName,
            "SetDOFFarCrisp",
            Lerp(progress, this.farCrispDistanceStart, this.farCrispDistanceEnd),
        );
        this.panel.FireEntityInput(
            this.cameraName,
            "SetDOFFarBlurry",
            Lerp(progress, this.farBlurryDistanceStart, this.farBlurryDistanceEnd),
        );
    }
}
...
class FireEntityInputAction extends RunFunctionAction {
    constructor(
        scenePanel: DOTAScenePanel_t,
        entityName: string,
        entityInput: string,
        entityInputValue: string | number,
    ) {
        super(() => {
...
class AnimateEntityInputAction implements ISequenceAction {
    ...
    endTimestamp = 0;
    constructor(
        public scenePanel: DOTAScenePanel_t,
