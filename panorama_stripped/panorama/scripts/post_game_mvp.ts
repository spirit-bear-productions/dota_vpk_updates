...
class AnimateMVP2TabAction extends RunSequentialActions {
    ...
    init(mvp2ScreenPanel: Panel_t): void {
        ...
        const addAccolade = function (
            nAccoladeIndex: number,
            accoladeObject: MVP2Accolade_t,
            accoladeContainer: Panel_t,
            wasDire: boolean,
        ) {
            ...
            if (accolade.ability_name != undefined) {
                (accolade_panel.FindChildInLayoutFile("ability_icon") as DOTAAbilityImage_t).abilityname =
                    accolade.ability_name;
            } else {
            ...
            if (accolade.particle_overlay != undefined) {
                const particle_overlay_panel: DOTAParticleScenePanel_t = accolade_panel.FindChildInLayoutFile(
                    "ParticleOverlay",
                ) as DOTAParticleScenePanel_t;
        ...
        const decorateHonorableMentionPanel = function (
            honorableMentionPanel: Panel_t,
            accoladeObject: MVP2Accolade_t,
        ) {
            const accolade_id = accoladeObject.type;
            const accolade = g_MVP_Accolade_TypeMap[accolade_id];
            ...
            if (accolade.particle_overlay != undefined) {
                const particle_overlay_panel: DOTAParticleScenePanel_t = honorableMentionPanel.FindChildInLayoutFile(
                    "ParticleOverlay",
                ) as DOTAParticleScenePanel_t;
                if (particle_overlay_panel) {
                    particle_overlay_panel.SetParticleNameAndRefresh(accolade.particle_overlay);
            ...
            new RunFunctionAction(function () {
                {
...

                    {
                        ...
                        econSetPreview.SetSticker(
                            mvpDetails.featured_hero_sticker_index,
                            mvpDetails.featured_hero_sticker_quality,
                        );
                    }
                ...
                for (let i = 0; i < 1 && i < mvp2Data.event_mvps.length; ++i) {
                    const honorableMentionPanel = setupHonorableMentionPanel(
                        eventHonorableMentions,
                        mvp2Data.event_mvps[i],
                    );
        ...
        if (this.data.bProgressVersion) this.actions.push(new SkippableAction(new WaitAction(0.5)));
        else this.actions.push(new WaitAction(0.5));
