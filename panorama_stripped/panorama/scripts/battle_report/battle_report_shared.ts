...
function SetSkippingReportAhead(bSkipAhead: boolean) {
    if (g_skipReportActions == bSkipAhead) return;
...
function StopSkippingReportAhead(): void {
    SetSkippingReportAhead(false);
}
function StartSkippingReportAhead(): void {
    SetSkippingReportAhead(true);
}
    ...
    for (let iRow = 0; iRow < nRows; ++iRow) {
        ...
        for (let iCol = 0; iCol < nCols; ++iCol) {
            ...
            if (nShardsEarned > 0) {
                ...
                par.actions.push(
                    new SkippableReportAction(
                        new AnimateDialogVariableIntAction(
                            screen,
                            "screen_shards_earned",
                            nShards,
