...
var Camera = function (cameraPosition: number) {
    if (cameraPosition === 0) {
        const seq = new RunSequentialActions();
        ($("#ModelBackground") as DOTAScenePanel_t).FireEntityInput(
            "loadout_camera_model",
            "SetAnimGraphParameter",
            "camera_anim=fullbody",
        );

        ($("#ModelBackgroundWK") as DOTAScenePanel_t).FireEntityInput(
            "loadout_camera_model",
            "SetAnimGraphParameter",
            "camera_anim=fullbody",
        );
        seq.actions.push(
            new LerpDepthOfFieldAction(
                $("#ModelBackground"),
                "hero_camera",
                200,
                300,
