...
function ShowAnnouncement(
    ...
    if (announcementsContainer != null) {
        ...
        heroImageFrame.SetImage(
            bPositive
                ? "file://{images}/hud/muerta/muerta_target_frame_radiant.psd"
                : "file://{images}/hud/muerta/muerta_target_frame_dire.psd",
        );
        ...
        announcementSequence.actions.push(
            new RunFunctionAction(function () {
                announcement.DeleteAsync(0.5);
            }),
        );
...
function AnimateScoreCounter(
    sequence: RunSequentialActions,
    counterPanel: Panel_t,
    counterLabel: Label_t,
    nNewScore: number,
    strScoredClass: string,
    fxBurstPanel: DOTAParticleScenePanel_t,
) {
...
function OnLocalPlayerScored(nNewScore: number, bKill: boolean, nHeroIdOverride: number) {
    ...
    sequence.actions.push(
        new PlaySoundAction(bKill ? "Muerta_ReleaseEvent.UI_Kill_Feedback" : "Muerta_ReleaseEvent.UI_Assist_Feedback"),
    );
    ...
    ShowAnnouncement(
        $.Localize(
            bKill
                ? "#DOTA_MuertaReleaseEvent_OpponentKillAnnouncementDesc"
                : "#DOTA_MuertaReleaseEvent_OpponentAssistAnnouncementDesc",
            mainPanel,
        ),
        $.Localize(
            bKill
                ? "#DOTA_MuertaReleaseEvent_AnnouncementKillReward"
                : "#DOTA_MuertaReleaseEvent_AnnouncementAssistReward",
            mainPanel,
        ),
        true,
    ...
    if (counterPanel != null) {
        ...
        AnimateScoreCounter(
            ...
            counterLabel,
            nNewScore,
            MuertaReleaseEventClasses.LocalPlayerScored,
            fxBurstPanel,
        );
...
function OnOpponentScored(nNewScore: number, bKill: boolean, nOpponentPlayerId: number) {
    ...
    if (counterPanel != null) {
        ...
        AnimateScoreCounter(
            ...
            counterLabel,
            nNewScore,
            MuertaReleaseEventClasses.OpponentScored,
            fxBurstPanel,
        );
...
function InitializeMuertaMinigamePanels(
    nLocalPlayerId: number,
    nTargetPlayerId: number,
    nLocalPlayerScore: number,
    nTargetPlayerScore: number,
) {
...
function ShowDescriptionPanel(nLocalHeroID: number, nTargetHeroID: number): void {
    ...
    if (mainPanel.BHasClass(MuertaReleaseEventClasses.ShowVersusPanel)) return;
    ...
    if (nLocalHeroID != g_nMuertaHeroID) {
        const voSoundEventName =
            nTargetHeroID == g_nMuertaHeroID
                ? "Muerta_ReleaseEvent.UI_InitialSlideIn_VO_TargetIsMuerta"
                : "Muerta_ReleaseEvent.UI_InitialSlideIn_VO";
...
function ShowGameEndPanel(
    nPlayer1Score: number,
    nPlayer2Score: number,
    nPlayer1Flowers: number,
    nPlayer2Flowers: number,
): void {
    const panel = $.GetContextPanel();
    const hero1Panel = $.GetContextPanel().FindChildTraverse("Hero1Panel") as Panel_t;
    const hero2Panel = $.GetContextPanel().FindChildTraverse("Hero2Panel") as Panel_t;
    ...
    hero1Frame.SetImage(
        bPlayer1Winner
            ? "file://{images}/events/muerta/versus_screen/portrait_frame_top_glow.png"
            : "file://{images}/events/muerta/versus_screen/portrait_frame_top.png",
    );
    hero2Frame.SetImage(
        bPlayer2Winner
            ? "file://{images}/events/muerta/versus_screen/portrait_frame_top_glow.png"
    ...
    winnerLabel1.text = $.Localize(
        bPlayer1Winner ? "#DOTA_MuertaReleaseEvent_EndGame_Points_Winner" : "#DOTA_MuertaReleaseEvent_EndGame_Points",
        winnerLabel1,
    );
    winnerLabel2.text = $.Localize(
        bPlayer2Winner ? "#DOTA_MuertaReleaseEvent_EndGame_Points_Winner" : "#DOTA_MuertaReleaseEvent_EndGame_Points",
        winnerLabel2,
    );
